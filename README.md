...
| `@shot.png` · a pasted image path | the image goes to the model with your message (PNG, JPEG, GIF, WebP; up to 4, 5 MB each), after the same read rules. Only that turn gets the image; the saved chat keeps a one-line note. A model without vision gets the note and Aegis tells you; with the Claude Code engine the image goes to Claude as an image block too (its stream-json input). Explorer's "Copy as path" (with quotes) works. The agent's own `read` of an image file shows it the image the same way. In Aegis Studio, paste (ctrl+v) or drop a screenshot into the message box |
| Studio `+` menu | next to the model picker, like Codex and Claude desktop: **Add photos & files** (images, and text files such as logs, configs, scripts and notes: up to 5, 200 KB each; secret-looking values are cut before the model sees them; PDF and Word files are not read yet), **Thinking** off/low/medium/high, **Reasoning** folded/shown/hidden, and **Plan mode**. The bar shows only what differs from the defaults |
