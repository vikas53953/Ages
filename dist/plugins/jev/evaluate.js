...
import { hasJevCredentials, jevApiKey } from "../../env.js";
import { DIFFICULTY_LABELS, TOOL_CLASSES, TURN_KINDS, } from "../../types.js";
import { TOOL_QUESTIONS, TURN_QUESTIONS } from "./questions.js";
...
function asKind(value) {
    return TURN_KINDS.includes(value) ? value : undefined;
}
function asClass(value) {
    return TOOL_CLASSES.includes(value) ? value : undefined;
}
function asDifficulty(score) {
...
export function failClosedTurn() {
    return {
        kind: "architecture",
        difficulty: 3,
...
export function failClosedTool() {
    return {
        class: "irreversible",
        dataLoss: 1,
        confidence: 0,
...
export async function evaluateTurn(state, abortSignal) {
    ...
    try {
        ...
        const { difficulty, difficultyLabel } = asDifficulty(result.answers.difficulty.score);
...
export async function evaluateTool(state, abortSignal) {
    if (abortSignal?.aborted)
