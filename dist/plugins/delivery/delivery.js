...
function namedTaskDir(cwd, id) {
    return path.join(harnessRoot(cwd), "tasks", safeTaskId(id));
}
export function slotDir(cwd, slot) {
    return slot.kind === "legacy" ? taskDir(cwd) : namedTaskDir(cwd, slot.id);
}
function taskFile(cwd, name, slot = { kind: "legacy" }) {
    return path.join(slotDir(cwd, slot), name);
}
function activeRefFile(cwd) {
    return path.join(harnessRoot(cwd), "active-task.json");
}
function pendingConfirmFile(cwd) {
    return path.join(harnessRoot(cwd), "pending-confirm.json");
}
function pendingDraftFile(cwd) {
    return path.join(harnessRoot(cwd), "pending-draft.json");
}
export async function readActiveSlot(cwd) {
    const stored = await readOptionalJson(activeRefFile(cwd));
    if (stored?.kind === "named" && stored.id)
        return { kind: "named", id: stored.id };
...
export async function writeActiveSlot(cwd, slot) {
    await writeJson(activeRefFile(cwd), slot);
}
export async function readPendingConfirm(cwd, sessionId) {
    const pending = await readOptionalJson(pendingConfirmFile(cwd));
    if (!pending)
        return undefined;
...
export async function writePendingConfirm(cwd, pending) {
    await writeJson(pendingConfirmFile(cwd), pending);
}
export async function clearPendingConfirm(cwd) {
    await unlinkQuiet(pendingConfirmFile(cwd));
}
export async function readPendingDraft(cwd, sessionId) {
    const draft = await readOptionalJson(pendingDraftFile(cwd));
    if (!draft)
        return undefined;
    if (sessionId !== undefined && draft.sessionId !== sessionId)
        return undefined;
    return draft;
}
export async function writePendingDraft(cwd, draft) {
    await writeJson(pendingDraftFile(cwd), draft);
}
export async function clearPendingDraft(cwd) {
    await unlinkQuiet(pendingDraftFile(cwd));
}
export async function clearConversationalPending(cwd) {
    await clearPendingConfirm(cwd);
    await clearPendingDraft(cwd);
}
export function agreementFromOwnerText(id, text) {
    ...
    return {
        ...
        acceptance: safeTaskId(id) === INVENTORY_TASK_ID
...
export function workDirForSlot(cwd, slot) {
    return slot.kind === "named" ? canonicalDir(path.join(cwd, "work", slot.id)) : canonicalDir(cwd);
}
export async function ensureWorkDir(cwd, slot) {
    const dir = workDirForSlot(cwd, slot);
    if (slot.kind === "named")
        await mkdir(dir, { recursive: true });
    return dir;
}
export async function writeSlotJson(cwd, name, value, slot) {
    await writeJson(taskFile(cwd, name, slot), value);
}
export async function readSlotJson(cwd, name, slot) {
    return readOptionalJson(taskFile(cwd, name, slot));
}
async function referenceBodies(cwd, references) {
    const bodies = {};
...
export async function agreementHash(agreement, cwd) {
...
async function hashPath(target, hash, root) {
    let info;
    try {
...
export async function captureIdentity(taskCwd, targetDir = taskCwd) {
...
export function parseAgreement(raw) {
    if (!raw || typeof raw !== "object")
        throw new Error("agreement is not an object");
    const row = raw;
    ...
    if (!isStringArray(row.scope))
        throw new Error("agreement.scope must be a string list");
    if (!Array.isArray(row.acceptance) || row.acceptance.length < 1) {
        throw new Error("agreement.acceptance is required");
    }
    const acceptance = row.acceptance.map((item, index) => {
    ...
    if (!isStringArray(row.exclusions))
        throw new Error("agreement.exclusions must be a string list");
...
export async function readAgreementState(cwd, slot = { kind: "legacy" }) {
...
async function checkStale(check, task, identity) {
    if (check.agreementHash !== task.fingerprint)
        return true;
    const criterion = task.agreement.acceptance.find((item) => item.id === check.criterionId);
    if (!criterion || check.criterionText !== criterion.text)
        return true;
    if (check.kind === "run" && check.executed) {
        if (!check.workdir || canonicalDir(check.workdir) !== canonicalDir(task.targetDir))
            return true;
    }
    if (check.identity.harnessHash !== identity.harnessHash)
        return true;
    if (check.identity.workspaceHash !== identity.workspaceHash)
        return true;
    return false;
}
export function isVerifiedCheck(check, criterion, task) {
    if (check.stale)
        return false;
    if (!check.executed)
        return false;
    ...
    if (check.agreementHash !== task.fingerprint)
        return false;
    if (check.criterionText !== criterion.text)
        return false;
    if (!check.workdir || canonicalDir(check.workdir) !== canonicalDir(task.targetDir))
        return false;
    return true;
}
export async function loadTask(cwd, slot) {
    ...
    if (state.kind === "invalid")
        throw new Error(`Broken delivery agreement: ${state.reason}`);
    const revisions = (await readOptionalJson(taskFile(cwd, "revisions.json", resolved))) ?? [];
    const checks = (await readOptionalJson(taskFile(cwd, "evidence.json", resolved))) ?? [];
    const meta = (await readOptionalJson(taskFile(cwd, "meta.json", resolved))) ?? {};
    ...
    task.checks = await Promise.all(checks.map(async (check) => ({ ...check, stale: await checkStale(check, task, identity) })));
    return task;
}
export async function taskPermission(cwd) {
...
export function isTerminalAgreementBlock(reason) {
    if (!reason)
        return false;
    return /unconfirmed delivery agreement|broken delivery agreement/.test(reason);
}
export async function deliveryMutationBlock(cwd) {
    const slot = await readActiveSlot(cwd);
    if (!slot)
        return undefined;
    const state = await readAgreementState(cwd, slot);
    ...
    const fingerprint = await agreementHash(state.agreement, cwd);
    if (state.agreement.status === "confirmed")
        return undefined;
...
export async function writeAgreement(cwd, agreement) {
    if (agreement.status === "confirmed") {
        throw new Error("Use confirmAgreement. Do not write a confirmed agreement directly.");
    }
    const slot = { kind: "legacy" };
    const proposed = { ...agreement, status: "proposed" };
...
export async function findSlotById(cwd, id) {
    const wanted = safeTaskId(id);
    const namedState = await readAgreementState(cwd, { kind: "named", id: wanted });
    if (namedState.kind !== "absent")
        return { kind: "named", id: wanted };
    const legacy = await readAgreementState(cwd, { kind: "legacy" });
    if (legacy.kind === "ok" && legacy.agreement.id === wanted)
        return { kind: "legacy" };
    return undefined;
}
export async function proposeNewTask(cwd, agreement, sessionId) {
    if (agreement.status === "confirmed") {
        throw new Error("Use confirmAgreement. Do not write a confirmed agreement directly.");
    }
    ...
    const slot = { kind: "named", id };
    const proposed = { ...agreement, id, status: "proposed" };
    delete proposed.confirmedAt;
    delete proposed.confirmedBy;
    await writeJson(taskFile(cwd, "agreement.json", slot), proposed);
    await writeJson(taskFile(cwd, "revisions.json", slot), []);
...
export async function confirmAgreement(cwd, actor, bound) {
    if (actor !== "owner")
        throw new Error("Only the owner can confirm the agreement.");
    const pending = await readPendingConfirm(cwd);
    let slot;
    if (bound?.id) {
        slot = await findSlotById(cwd, bound.id);
        if (!slot)
            throw new Error(`No task '${bound.id}'.`);
    }
    else if (pending) {
        slot = pending.slot;
    }
    ...
    if (!slot)
        throw new Error("No task agreement.");
    ...
    if (pending && !bound?.id && (pending.id !== task.agreement.id || pending.fingerprint !== task.fingerprint)) {
        throw new Error(`Displayed agreement ${pending.id} hash ${pending.fingerprint} no longer matches ${task.agreement.id} hash ${task.fingerprint}. Review /task.`);
    }
    const expected = bound?.fingerprint ??
        (pending && (!bound?.id || pending.id === bound.id) ? pending.fingerprint : undefined);
    if (expected && expected !== task.fingerprint) {
        throw new Error(`Refusing to confirm '${task.agreement.id}'. Displayed hash ${expected}, current hash ${task.fingerprint}.`);
    }
    const confirmed = {
        ...task.agreement,
        status: "confirmed",
...
export function assertReadyToImplement(task) {
    if (task.agreement.status !== "confirmed") {
        throw new Error("Owner has not confirmed the agreement.");
    }
}
export async function reviseAgreement(cwd, next, input) {
    if (!input.reason.trim())
        throw new Error("Revisions need a reason.");
    ...
    const refsChanged = JSON.stringify([...(task.agreement.references ?? [])].sort()) !==
...
async function appendCheck(cwd, record) {
    const task = await loadTask(cwd);
    const slot = task?.slot ?? { kind: "legacy" };
    const previous = (await readOptionalJson(taskFile(cwd, "evidence.json", slot))) ?? [];
    previous.push(record);
    await writeJson(taskFile(cwd, "evidence.json", slot), previous);
    return record;
}
async function criterionFor(cwd, criterionId) {
    const task = await loadTask(cwd);
    if (!task)
        throw new Error("No task agreement.");
...
function runArgv(argv, cwd, timeoutMs, abortSignal) {
    return runOwnedArgv(argv, cwd, { timeoutMs, abortSignal });
}
export async function runCheck(cwd, input) {
    ...
    if (workdir !== task.targetDir) {
        throw new Error(`Check workdir must be the agreed task directory (${task.targetDir}); got ${workdir}. A zero exit from another project is not this project's evidence.`);
    }
...
export async function reportCheck(cwd, input) {
...
export async function recordMissing(cwd, input) {
    const { criterion, hash, identity } = await criterionFor(cwd, input.criterionId);
    return appendCheck(cwd, {
        criterionId: input.criterionId,
        criterionText: criterion.text,
...
export async function recordCheck(_cwd, _input) {
    throw new Error("Use runCheck for verification. Descriptions cannot set passed.");
}
export async function noteChanged(cwd, files) {
    const task = await loadTask(cwd);
    if (!task)
        throw new Error("No task agreement.");
...
export async function addScreenshot(cwd, shot) {
    const task = await loadTask(cwd);
    if (!task)
        throw new Error("No task agreement.");
    await writeJson(taskFile(cwd, "meta.json", task.slot), {
...
export async function markImplemented(cwd) {
    const task = await loadTask(cwd);
    if (!task)
        throw new Error("No task agreement.");
...
export async function acceptTask(cwd, actor) {
    if (actor !== "owner")
        throw new Error("Only the owner can record acceptance.");
    const task = await loadTask(cwd);
    if (!task)
        throw new Error("No task agreement.");
    if (task.agreement.status !== "confirmed") {
...
export async function setDeliveryFlags(cwd, flags) {
    const task = await loadTask(cwd);
    if (!task)
        throw new Error("No task agreement.");
    await writeJson(taskFile(cwd, "meta.json", task.slot), {
...
function latestChecks(task) {
    const map = new Map();
    for (const check of task.checks)
        map.set(check.criterionId, check);
    return map;
}
export function renderCard(task) {
    ...
    for (const item of task.agreement.acceptance) {
        ...
        if (isVerifiedCheck(check, item, task))
            verified.push(`- ${item.id}: ${result}`);
        else
            notVerified.push(`- ${item.id}: ${result}`);
    }
...
export function formatAgreementPrompt(task) {
    return [
...
export async function formatTaskSystemPrompt(cwd) {
...
export async function formatTaskStatus(cwd, sessionId) {
    ...
    if (!activeSlot) {
        ...
        if (state.kind === "ok") {
            const fingerprint = await agreementHash(state.agreement, cwd);
            lines.push(`Active task  ${state.agreement.id}  ${state.agreement.status}  hash ${fingerprint}`, `Objective  ${state.agreement.objective.split(/\r?\n/)[0].slice(0, 160)}`);
        }
    ...
    if (pending) {
        lines.push(`Pending confirm  ${pending.id}  hash ${pending.fingerprint}`, `Reply yes now to confirm that exact version, or /task confirm ${pending.id} ${pending.fingerprint}. An intervening prompt, /new, /clear, or restart cancels yes.`);
    }
    else {
        lines.push("Pending confirm  (none)");
        const diskPending = sessionId ? await readOptionalJson(pendingConfirmFile(cwd)) : undefined;
        if (diskPending && sessionId && diskPending.sessionId !== sessionId) {
            lines.push(`A proposal '${diskPending.id}' exists from another session. Plain yes will not confirm it. Re-review and /task confirm ${diskPending.id} ${diskPending.fingerprint}.`);
        }
        else if (activeSlot) {
            const state = await readAgreementState(cwd, activeSlot);
            if (state.kind === "ok" && state.agreement.status === "proposed") {
                const fingerprint = await agreementHash(state.agreement, cwd);
                lines.push(`Plain yes will not confirm this. Use /task confirm ${state.agreement.id} ${fingerprint} or /task new <id> for a different task.`);
            }
        }
    }
    if (draft) {
...
export async function renderProposalReview(cwd, slot) {
    const proposed = await loadTask(cwd, slot);
    if (!proposed)
        throw new Error("No proposed task to review.");
    const activeSlot = await readActiveSlot(cwd);
    ...
    if (!sameActive && activeSlot) {
        ...
        if (active) {
            lines.push("", "Preserved active task (unchanged — this is not what yes confirms)", `${active.agreement.id}  ${active.agreement.status}  hash ${active.fingerprint}`, `Objective  ${active.agreement.objective.split(/\r?\n/)[0].slice(0, 160)}`);
        }
    }
    lines.push("", `Reply yes now to confirm '${proposed.agreement.id}' hash ${proposed.fingerprint}, or /task confirm ${proposed.agreement.id} ${proposed.fingerprint}.`);
    return lines.join("\n");
}
export async function renderTaskCard(cwd, sessionId) {
