import { isExactYes } from "../../commands.js";
import { isMutation } from "../../rules.js";
import { appendMessage } from "../../session.js";
import { formatPlan, loadPlan, openTestedResult, runDeliveryBuild } from "./controller.js";
import { acceptTask, agreementFromOwnerText, clearConversationalPending, clearPendingConfirm, clearPendingDraft, confirmAgreement, deliveryMutationBlock, formatTaskStatus, formatTaskSystemPrompt, loadTask, proposeNewTask, readPendingConfirm, readPendingDraft, renderProposalReview, renderTaskCard, taskPermission, writePendingDraft, } from "./delivery.js";
/** Parse the words after /task. Undefined means "not a task command": the line goes to the model instead. */
export function parseTaskArg(arg) {
    if (!arg)
        return {};
    if (/^new$/i.test(arg))
        return { action: "new" };
    if (/^new\s+/i.test(arg))
        return { action: "new", id: arg.slice(4).trim() };
...
function reply(ctx, output) {
    return { output, session: ctx.state.session };
}
async function taskCommand(cmd, ctx) {
    const { state, opts, confirm, onEvent } = ctx;
    try {
        if (cmd.action === "new") {
            if (!cmd.id)
                return reply(ctx, "usage: /task new <id>");
...
async function interceptPrompt(text, ctx) {
    const { state } = ctx;
    ...
    if (draft) {
        try {
            ...
            return reply(ctx, [`Proposed '${proposed.agreement.id}' hash ${proposed.fingerprint}. Active task was not overwritten.`, review].join("\n"));
        }
        catch (error) {
            return reply(ctx, error instanceof Error ? error.message : String(error));
        }
 ...
 */
export function deliveryPlugin() {
    return {
        name: "delivery",
        help: [
        ...
        beforeTurn: async ({ state }) => {
            if (await readPendingConfirm(state.cwd, state.session.id))
                await clearPendingConfirm(state.cwd);
            if (await readPendingDraft(state.cwd, state.session.id))
                await clearPendingDraft(state.cwd);
        },
