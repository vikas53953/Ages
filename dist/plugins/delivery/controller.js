...
import { INVENTORY_ACCEPTANCE, INVENTORY_TASK_ID, captureIdentity, formatTaskSystemPrompt, isVerifiedCheck, loadTask, readSlotJson, recordMissing, renderCard, runCheck, setDeliveryFlags, writeSlotJson, } from "./delivery.js";
...
export function emptyPlan(task) {
    return {
...
export async function loadPlan(cwd, task) {
    const stored = await readSlotJson(cwd, "plan.json", task.slot);
    if (!stored)
        return undefined;
...
export async function loadOrCreatePlan(cwd, task) {
    return (await loadPlan(cwd, task)) ?? emptyPlan(task);
}
export async function savePlan(cwd, task, plan) {
...
export async function openTestedResult(cwd) {
    ...
    if (identity.workspaceHash !== plan.testedWorkspaceHash) {
        throw new Error(`Open refused: workspace hash ${identity.workspaceHash} does not match tested ${plan.testedWorkspaceHash}. Evidence is stale.`);
    }
...
function parseOpen(output, targetDir) {
...
async function runNodeCheck(cwd, task, node, abortSignal) {
...
async function allVerified(cwd) {
...
export async function runDeliveryBuild(input) {
    ...
    if (task.slot.kind !== "named" || task.agreement.id !== INVENTORY_TASK_ID) {
        throw new Error(`Delivery loop is the device-inventory proving ground. Active task is '${task.agreement.id}'. Use /task new ${INVENTORY_TASK_ID}.`);
    }
