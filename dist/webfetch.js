...
function isPublicIPv4(ip) {
    ...
    if (a === 192 && b === 0 && (c === 0 || c === 2))
        return false; // IETF, documentation
    if (a === 198 && (b === 18 || b === 19))
...
export function isPublicAddress(address) {
    ...
    if (zeros(10) && b[10] === 0xff && b[11] === 0xff)
        return isPublicIPv4(b.slice(12).join(".")); // ::ffff:a.b.c.d
    if (zeros(12))
        return false; // ::, ::1, IPv4-compatible ::a.b.c.d
    const w0 = (b[0] << 8) | b[1];
    const w1 = (b[2] << 8) | b[3];
    if (w0 === 0x64 && w1 === 0xff9b)
        return false; // NAT64 (64:ff9b::/96, 64:ff9b:1::/48) reaches IPv4 space
    if (w0 === 0x2002)
    ...
    if (b[0] === 0xfe && (b[1] & 0xc0) === 0xc0)
        return false; // old site-local fec0::/10
    if (b[0] === 0xff)
    ...
    for (let hop = 0; hop <= MAX_REDIRECTS; hop += 1) {
        ...
        if (response.status >= 300 && response.status < 400 && response.headers.location) {
            ...
            if (next.username || next.password)
                return { ok: false, reason: "redirected to a URL with a user name or password; not followed" };
