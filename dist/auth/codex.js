...
export function accountIdFrom(access) {
    const auth = decodeJwt(access)?.[JWT_AUTH_CLAIM];
    return typeof auth?.chatgpt_account_id === "string" && auth.chatgpt_account_id ? auth.chatgpt_account_id : undefined;
}
function toCredential(json, previousRefresh) {
    ...
    if (!accountId)
        throw new Error("ChatGPT sign-in: no ChatGPT account in the token (is this a ChatGPT plan?)");
    const email = json.id_token ? decodeJwt(json.id_token)?.email : undefined;
...
export async function loginCodexDevice(ui, options = {}) {
    ...
    if (!start.ok) {
        throw new Error(start.status === 404
    ...
    if (!device.device_auth_id || !device.user_code)
        throw new Error("ChatGPT sign-in: unexpected reply from auth.openai.com");
    ...
    while (Date.now() < deadline) {
        ...
        if (poll.ok) {
            ...
            if (!code.authorization_code || !code.code_verifier)
                throw new Error("ChatGPT sign-in: unexpected reply while waiting");
...
export async function loginCodexBrowser(ui, options = {}) {
    ...
    const page = (text) => `<!doctype html><meta charset="utf-8"><title>Aegis</title><body style="font:16px system-ui;padding:40px">${text}</body>`;
    ...
    try {
        ...
        if (!answer.code)
            throw new Error(`ChatGPT sign-in failed: ${answer.error}`);
        const tokens = await tokenRequest({ grant_type: "authorization_code", code: answer.code, code_verifier: verifier, redirect_uri: redirectUri }, ui.signal);
        return toCredential(tokens);
    }
    finally {
...
export async function refreshCodex(credential, signal) {
...
export async function freshCodexCredential(options = {}) {
 ...
 */
export function codexRequestBody(body) {
...
export function codexFetch(base = fetch) {
