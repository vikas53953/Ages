...
function notAValue(raw, name) {
    ...
    if (!/^[A-Z0-9_$"']/.test(name) || /[a-z]/.test(name)) {
        if (/^[a-z_][A-Za-z0-9_]*$/.test(value) && /[A-Z_]/.test(value))
            return true;
        if (["true", "false", "null", "undefined", "string", "number", "boolean"].includes(value))
            return true;
    }
...
function redactAll(text) {
    ...
    out = out.replace(SETTING, (match, name, between, value) => {
        if (!secretName(name) || notAValue(value, name))
            return match;
        count += 1;
