...
export function initialJevHealth(mockJev, mode = "second-opinion") {
    if (mode === "off")
        return "off";
    if (mockJev)
        return "mock";
...
export function jevHealthFromReceipt(mockJev, receipt) {
    if (receipt.turn.source === "off")
        return "off";
    if (mockJev)
        return "mock";
    if (!hasJevCredentials())
        return "blocked";
    if (receipt.turn.source === "fail_closed")
        return "down";
    if (receipt.tools.some((tool) => tool.source === "fail_closed"))
        return "down";
    if (receipt.tools.some((tool) => tool.source === "agreement")) {
        return receipt.turn.source === "jev" ? "live" : receipt.turn.source === "mock" ? "mock" : "live";
    }
    if (receipt.turn.source === "jev")
        return "live";
    if (receipt.turn.source === "mock")
        return "mock";
    return "down";
}
