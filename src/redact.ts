 ...
 */
function codeAfter(after: string, between: string) {
  if (/^\s*(?:[,)}]|\?\?|\|\||&&|!)/.test(after)) return true;
  if (after.startsWith(";")) return between.includes(":") || /^;\s*$/.test(after);
  return false;
}
 ...
 */
function notAValue(value: string, name: string, quoted: boolean, code: boolean) {
  if (PLACEHOLDER.test(value) || KEYWORDS.has(value.toLowerCase())) return true;
  ...
  if (!quoted) {
    ...
    if (value.toLowerCase() === bareName.toLowerCase()) return true;
    if (/[a-z]/.test(bareName) && /^[a-z]+(?:_[a-z]+)+$/.test(value)) return true;
  }
  if (!quoted) {
    ...
    if (code && !lowerWithDigit && /^[A-Za-z_$][\w$]*(?:<.*>)?(?:\[\])?$/.test(value)) return true;
    ...
    if (/[a-z]/.test(name.replace(/^\$(?:env:)?/, ""))) {
      if (/^[A-Z][a-z]+(?:[A-Z][a-z]+)*$/.test(value) || /^[A-Z][A-Z0-9]*(?:_[A-Z0-9]+)+$/.test(value)) return true;
