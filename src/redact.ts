 ...
 */
function codeAfter(after: string, between: string, before: string) {
 ...
 */
function notAValue(value: string, name: string, quoted: boolean, code: boolean, connection = false) {
  ...
  if (!quoted) {
    ...
    if (/[a-z]/.test(bareName) && !/^_auth/i.test(bareName) && /^[a-z]+(?:_[a-z]+)+$/.test(value)) return true;
  }
  if (!quoted) {
