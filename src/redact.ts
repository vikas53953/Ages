 ...
 */
const SETTING =
  /(?<![A-Za-z0-9_$])((?:\$env:|\$)?["']?[A-Za-z][A-Za-z0-9_-]{1,80}["']?)([ \t]*(?::=|=>|[=:])[ \t]*)(?:"([^"\n]{8,4096})"|'([^'\n]{8,4096})'|([^\s"'#,;`)\]}]{8,4096}))/g;
...
function notAValue(value: string, name: string, quoted: boolean, code: boolean) {
  ...
  if (!quoted) {
    ...
    if (code && /^[A-Za-z_$][\w$]*(?:<.*>)?(?:\[\])?$/.test(value)) return true;
...
function redactAll(text: string): { text: string; count: number } {
  ...
  out = out.replace(
    SETTING,
    (match: string, name: string, between: string, dq: string | undefined, sq: string | undefined, bare: string | undefined, offset: number, whole: string) => {
      const value = dq ?? sq ?? bare ?? "";
      const quote = dq !== undefined ? '"' : sq !== undefined ? "'" : "";
      const next = whole[offset + match.length] ?? "";
      if (!secretName(name) || notAValue(value, name, Boolean(quote), /[,;)]/.test(next))) return match;
      count += 1;
