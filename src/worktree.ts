...
export async function openWorktree(cwd: string, name: string): Promise<{ path: string; branch: string; created: boolean }> {
  ...
  if (existing && !existing.isDirectory()) throw new Error(`${folder} already exists and is not a worktree of this repository`);
  ...
  try {
    ...
    const lines = String((error as { stderr?: string }).stderr ?? "").split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
