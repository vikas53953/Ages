export type ModelApi = "chat" | "responses" | "messages" | "gemini" | "systemone";
...
export function inferEntry(id: string): CatalogEntry {
  const known = byId.get(id);
  if (known) return known;
  if (id.startsWith("jev-")) return { id, name: prettyName(id), group: "Jev", api: "systemone" };
  ...
  if (id.startsWith("gemini-")) return { id, name: prettyName(id), group: "Google", api: "gemini" };
...
export function findModel(id: string, rows: CatalogEntry[] = KNOWN) {
...
export function resolveModel(
  query: string,
  rows: CatalogEntry[] = KNOWN,
): { ok: true; id: string } | { ok: false; message: string } {
  const needle = normalizeModelId(query);
  if (!needle) return { ok: false, message: "usage: /model <id>" };
  ...
  if (named.length === 1) return { ok: true, id: named[0].id };
...
export function mergeCatalog(ids: string[]): CatalogEntry[] {
...
export function formatModelList(current: string, rows: CatalogEntry[] = KNOWN) {
  const lines = [
...
export async function fetchZenModelIds(fetcher: typeof fetch = fetch): Promise<string[]> {
