...
export function cleanTodos(value: unknown): Todo[] {
  const rows = Array.isArray(value) ? value : [];
  const out: Todo[] = [];
  for (const row of rows.slice(0, MAX_TODOS)) {
    const item = row as { content?: unknown; status?: unknown };
    const content = String(item?.content ?? "")
      ...
      .slice(0, MAX_TEXT);
    if (!content) continue;
    const status = STATUSES.has(item?.status as TodoStatus) ? (item.status as TodoStatus) : "pending";
...
export function todoLines(todos: Todo[], max = 5) {
  if (!todos.length || todos.every((todo) => todo.status === "completed" || todo.status === "cancelled")) return [];
...
export function todosFromMessages(rows: ChatMessage[]): Todo[] {
  for (let i = rows.length - 1; i >= 0; i -= 1) {
    ...
    for (let j = row.content.length - 1; j >= 0; j -= 1) {
      ...
      if (part.type === "tool-call" && part.toolName === "todo") return cleanTodos((part.input as { todos?: unknown })?.todos);
    }
  }
  return [];
}
