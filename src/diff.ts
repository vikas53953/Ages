...
function operations(a: string[], b: string[]): Op[] {
  ...
  const ops: Op[] = [];
  for (let i = 0; i < start; i += 1) ops.push({ kind: " ", text: a[i]!, oldLine: i + 1, newLine: i + 1 });
  ...
  for (let k = 0; k < a.length - endA; k += 1) ops.push({ kind: " ", text: a[endA + k]!, oldLine: endA + k + 1, newLine: endB + k + 1 });
  return ops;
}
...
export function unifiedDiff(oldText: string, newText: string, context = 3): { lines: string[]; stat: DiffStat } {
  ...
  while (index < changed.length) {
    ...
    while (end + 1 < changed.length && changed[end + 1]! - changed[end]! <= 2 * context + 1) end += 1;
