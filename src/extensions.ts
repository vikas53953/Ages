...
export type CommandEntry = { name: string; description: string; file: string; scope: Scope; source: string; argumentHint?: string };
/** A custom agent (Claude Code's agents/<name>.md format): its own instructions and tool list, behind the same lock. */
export type AgentEntry = {
  ...
  scope: Scope;
  source: string;
  /** Aegis tool names it may use; read, grep and glob when the file names none. */
  tools: string[];
...
function roots(cwd: string) {
  ...
  return {
    skills: [
      { dir: path.join(cwd, ".aegis", "skills"), scope: "project" as const, source: ".aegis/skills" },
      { dir: path.join(cwd, ".agents", "skills"), scope: "project" as const, source: ".agents/skills" },
      { dir: path.join(cwd, ".claude", "skills"), scope: "project" as const, source: ".claude/skills" },
      { dir: path.join(userAegisDir(), "skills"), scope: "user" as const, source: "~/.aegis/skills" },
      { dir: path.join(home, ".agents", "skills"), scope: "user" as const, source: "~/.agents/skills" },
      { dir: path.join(home, ".claude", "skills"), scope: "user" as const, source: "~/.claude/skills" },
    ],
    commands: [
      { dir: path.join(cwd, ".aegis", "commands"), scope: "project" as const, source: ".aegis/commands" },
      { dir: path.join(userAegisDir(), "commands"), scope: "user" as const, source: "~/.aegis/commands" },
    ],
    agents: [
      { dir: path.join(cwd, ".aegis", "agents"), scope: "project" as const, source: ".aegis/agents" },
      { dir: path.join(cwd, ".claude", "agents"), scope: "project" as const, source: ".claude/agents" },
      { dir: path.join(userAegisDir(), "agents"), scope: "user" as const, source: "~/.aegis/agents" },
      { dir: path.join(home, ".claude", "agents"), scope: "user" as const, source: "~/.claude/agents" },
    ],
  };
}
...
async function scanSkills(cwd: string) {
...
async function scanCommands(cwd: string) {
...
async function scanAgents(cwd: string) {
  ...
  for (const root of roots(cwd).agents) {
    ...
    for (const fileName of names) {
      ...
      if (!NAME.test(name) || !body.trim()) continue;
      const listed = typeof data.tools === "string"
        ? data.tools.split(/[,\s]+/).map((tool) => AGENT_TOOLS[tool.trim().toLowerCase()]).filter((tool): tool is string => Boolean(tool))
        : [];
      found.push({
        name,
...
export type Extensions = {
  ...
  agents: AgentEntry[];
  /** Project skills/commands exist but are not trusted (as they are now). */
  untrustedProject: number;
};
...
export async function loadExtensions(cwd: string): Promise<Extensions> {
...
export async function trustProjectExtensions(cwd: string) {
...
export function skillsPromptBlock(skills: SkillEntry[]) {
...
async function listFiles(dir: string, max = 20) {
  const out: string[] = [];
  const walk = async (current: string, depth: number) => {
    ...
    for (const entry of await readdir(current, { withFileTypes: true }).catch(() => [])) {
      ...
      else if (entry.isFile() && entry.name !== "SKILL.md") out.push(path.relative(dir, full).replaceAll("\\", "/"));
    }
  };
  await walk(dir, 0);
  return out;
...
export async function readSkill(skills: SkillEntry[], name: string, file?: string) {
  const skill = skills.find((row) => row.name === String(name).toLowerCase());
  if (!skill) return `No skill named ${name}. Available: ${skills.map((row) => row.name).join(", ") || "none"}`;
  const open = (text: string, label: string) => `<skill name="${skill.name}" source="${skill.source}" file="${label}">\n${text}\n</skill>`;
  if (file) {
    if (path.isAbsolute(file) || /^\\\\/.test(file) || file.split(/[\\/]/).includes("..")) return "That file is outside the skill folder.";
    const root = await realpath(skill.dir);
    const target = await realpath(path.join(skill.dir, file)).catch(() => undefined);
    if (!target) return `No file ${file} in skill ${skill.name}.`;
    const inside = path.relative(root, target);
    if (inside.startsWith("..") || path.isAbsolute(inside)) return "That file is outside the skill folder.";
  ...
  if (text === undefined) return `Skill ${skill.name} could not be read.`;
...
export function extensionsExist(cwd: string) {
  const all = roots(cwd);
