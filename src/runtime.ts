...
import { addMemory, loadMemory, memoryNotes, removeMemory } from "./memory.ts";
...
export type RunOpts = {
  ...
  unattended?: boolean;
  /** Text files you attached in Aegis Studio for this message: sent as text (secret-looking values cut). */
...
export type AppState = {
  ...
  /** The hash of the project settings /trust just showed you: /trust yes trusts exactly those bytes. */
  trustOffer?: string;
...
async function forkCommand(state: AppState, arg: string | undefined): Promise<HandleResult> {
  const drop = arg === undefined ? 0 : Number(arg);
  if (!Number.isInteger(drop) || drop < 0) return { output: "usage: /fork, or /fork <n> to leave out your last n turns", session: state.session };
  ...
  if (drop > starts.length) return { output: `There are only ${starts.length} turn(s) to leave out.`, session: state.session };
  ...
  const claudeNote =
    drop && existsSync(path.join(source, "claude-session"))
...
function untrustedNotice(state: AppState, trust: ProjectTrust | undefined) {
  if (!trust || trust.trusted || !trust.ignored.length || state.trustNoticeShown) return "";
...
function attachedDocuments(documents: Array<{ name: string; text: string }>) {
...
async function diffCommand(state: AppState, arg: string) {
  ...
  const picked = (name: string) => !wanted || same(name, wanted) || (!exact && same(name.slice(-(wanted.length + 1)), `/${wanted}`));
  const out: string[] = [];
  let total = 0;
  let shown = 0;
  ...
  if (/^(remove|rm|delete|del)$/i.test(verb)) {
    ...
    const row = byText
      ...
      : Number.isInteger(n)
        ? rows[n - 1]
        : undefined;
    if (!row) return `usage: /rules remove <n> (1 to ${rows.length}), or /rules remove allow <rule> as /rules shows it`;
    if (row.source !== "yours") {
...
function trustCommand(state: AppState, action: string | undefined) {
  ...
  const loaded = loadSettingsSafe(state.cwd);
  if (loaded.error) return `Fix ${file} first: ${loaded.error}`;
  const trust = loaded.trust!;
  if (!trust.exists) return `No ${file} here, so there is nothing to trust. Your own choices are kept in ${yourSettingsPath(state.cwd)}.`;
  if (action === "off") {
  ...
  if (action === "yes") {
    if (!state.trustOffer) return "Type /trust first to see what you would be trusting.";
  ...
  if (trust.trusted) return `${file} is trusted${process.env.AEGIS_TRUST_PROJECT === "1" ? " (AEGIS_TRUST_PROJECT=1)" : ""}. /trust off to stop.`;
