...
/**
 * A server Aegis starts (command, args, env, cwd: stdio) or one it talks to over HTTP (url, headers: MCP's
 * "streamable HTTP"). Header values may use ${NAME} for an environment variable, in YOUR settings only.
 */
...
function serversIn(file: string): Record<string, McpServerConfig> {
  ...
  for (const [name, server] of Object.entries(mcp?.servers ?? {})) {
    if (!NAME.test(name) || !server || typeof server !== "object") continue;
    if (typeof server.url === "string" && server.url) {
      out[name] = { url: server.url, headers: strings(server.headers) };
      continue;
    }
    if (typeof server.command !== "string" || !server.command) continue;
...
export abstract class McpClient {
  ...
  abstract request(method: string, params: object, timeoutMs?: number, signal?: AbortSignal): Promise<unknown>;
  protected abstract notify(method: string, params?: object): void;
  abstract close(): void;
...
export class McpConnection extends McpClient {
  ...
  protected notify(method: string, params?: object) {
...
export function mcpUrlProblem(raw: string) {
  let url: URL;
  try {
    url = new URL(raw);
  } catch {
  ...
  if (url.username || url.password) return "has a user name or password in it; put a token in headers instead";
...
function expandHeaders(serverName: string, headers: Record<string, string> | undefined, expand: boolean) {
  const out: Record<string, string> = {};
  for (const [key, value] of Object.entries(headers ?? {})) {
    const text = expand
      ? value.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*)\}/g, (_, name: string) => {
          const found = process.env[name];
          if (found === undefined || found === "") throw new Error(`MCP server ${serverName}: header ${key} needs \${${name}}, which is not set`);
          return found;
        })
      : value;
    // Checked here, so the error names the header and never repeats its value (a token) the way fetch would.
    if (/[\r\n\0]/.test(text)) throw new Error(`MCP server ${serverName}: header ${key} has a line break or NUL in it`);
    out[key] = text;
  }
  return out;
}
...
export class McpHttpConnection extends McpClient {
  ...
  constructor(name: string, server: McpServerConfig, expandEnv: boolean) {
    super(name);
    const problem = mcpUrlProblem(server.url ?? "");
    if (problem) throw new Error(`MCP server ${name}: the url ${problem}`);
    this.url = server.url!;
    this.headers = expandHeaders(name, server.headers, expandEnv);
  }

  private post(body: object, signal: AbortSignal) {
  ...
  async request(method: string, params: object, timeoutMs = 60_000, signal?: AbortSignal): Promise<unknown> {
    ...
    try {
      ...
      if (method === "initialize") this.session = response.headers.get("mcp-session-id") ?? undefined;
      ...
      if (signal?.aborted && !this.closed) this.notify("notifications/cancelled", { requestId: id, reason: "stopped" });
    }
  }

  /** The JSON-RPC answer with this id, from a JSON body or an event stream; at most MAX_HTTP_BODY bytes read. */
  private async answer(response: Response, id: number): Promise<{ result?: unknown; error?: { message?: string } }> {
    const type = response.headers.get("content-type") ?? "";
    const reader = response.body?.getReader();
    if (!reader) throw new Error(`MCP server ${this.name}: empty answer`);
    ...
    const pick = (value: unknown) => {
      ...
      return list.find((item) => item && typeof item === "object" && (item as { id?: unknown }).id === id && ("result" in item || "error" in item)) as
    ...
    try {
      for (;;) {
        ...
        if (value) {
          bytes += value.length;
          if (bytes > MAX_HTTP_BODY) throw new Error(`MCP server ${this.name}: answer over ${MAX_HTTP_BODY / 1_000_000} MB`);
      ...
      if (!type.includes("text/event-stream")) {
        if (!text.trim()) throw new Error(`MCP server ${this.name}: empty answer to request ${id}`);
  ...
  close() {
    if (this.closed) return;
    this.closed = true;
    for (const abort of this.aborts) abort.abort(new Error(`MCP server ${this.name} closed`));
...
export async function startMcp(cwd: string): Promise<McpState> {
  ...
  for (const server of mcpServers(cwd)) {
    ...
    let connection: McpClient | undefined;
    try {
