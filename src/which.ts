...
export function findOnPath(name: string, env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  if (name.includes("/") || name.includes("\\")) return isFile(name) ? path.resolve(name) : undefined;
...
export function programPath(name: string) {
  ...
  if (!found) throw new Error(`${name} was not found on PATH`);
  return found;
}

/** A Windows tool from System32 by full path (taskkill, icacls, rundll32, where). */
export function system32(name: string) {
...
export const NO_CWD_SEARCH_ENV = { NoDefaultCurrentDirectoryInExePath: "1" } as const;
