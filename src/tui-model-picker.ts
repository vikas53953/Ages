import { on } from "./theme.ts";
import { CURSOR_MARKER, Key, matchesKey, type Component, type Focusable } from "@earendil-works/pi-tui";

export type PickerItem = { id: string; group: string; note: string };
...
export function filterItems(items: PickerItem[], query: string) {
  ...
  const rank = (item: PickerItem) => {
    ...
    if (words.every((word) => id.includes(word))) return 2;
    return 3;
  };
  return items
    .filter((item) => {
...
export class ModelPicker implements Component, Focusable {
  ...
  constructor(
    ...
    private readonly onDone: (id: string | undefined) => void,
  ...
  handleInput(data: string) {
    ...
    if (matchesKey(data, Key.enter)) return this.onDone(list[this.index]?.id);
  ...
  render(width: number): string[] {
    ...
    if (!list.length) lines.push(` ${on("dim")}No model matches "${this.query}"${RESET}`);
